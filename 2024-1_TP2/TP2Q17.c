...
// Implementação da função de comparação para o Shellsort
bool isBigger3(String str1, String str2, Personagem pivo, Personagem p)
{
    ...
    if (eyeColorComparison != 0) {
        // Se as cores dos olhos forem diferentes, comparar apenas com base na cor dos olhos
        return eyeColorComparison > 0;
    } else {
        // Se as cores dos olhos forem iguais, desempatar por nome
        return strcmp(pivo.name, p.name) > 0;
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    // Comparar a cor do cabelo dos personagens
bool isBigger4(String str1, String str2, Personagem pivo, Personagem p)
{
    return strcmp(pivo.hairColour, p.hairColour) > 0;
}

// Transformar uma subárvore em um heap
void heapify(String *idArray, int n, int i)
{
    ...
    // Comparar os elementos com base na cor do cabelo e, em caso de empate, pelo nome
    if (left < n && (isBigger4(getElementByID(idArray[left]).hairColour, getElementByID(idArray[largest]).hairColour, getElementByID(idArray[left]), getElementByID(idArray[largest])) ||
        (strcmp(getElementByID(idArray[left]).hairColour, getElementByID(idArray[largest]).hairColour) == 0 &&
         isBigger2(getElementByID(idArray[left]).name, getElementByID(idArray[largest]).name, getElementByID(idArray[left]), getElementByID(idArray[largest])))))
    {
        largest = left;
    }

    if (right < n && (isBigger4(getElementByID(idArray[right]).hairColour, getElementByID(idArray[largest]).hairColour, getElementByID(idArray[right]), getElementByID(idArray[largest])) ||
        (strcmp(getElementByID(idArray[right]).hairColour, getElementByID(idArray[largest]).hairColour) == 0 &&
         isBigger2(getElementByID(idArray[right]).name, getElementByID(idArray[largest]).name, getElementByID(idArray[right]), getElementByID(idArray[largest])))))
    {
        largest = right;
    }
    ...
    if (largest != i)
    {
        swap(idArray, i, largest);

        // Recursivamente heapify a subárvore afetada
        heapify(idArray, n, largest);
    }
}


// Função principal para ordenar um array de strings
void heapSort(String *idArray, int n)
{
    // Construir um heap (rearranjar o array)
   ...
   // ----------------- Realizar a ordenacao com Heapsort --------------------------
heapSort(idArray, x);
