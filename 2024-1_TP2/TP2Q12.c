...
// Função para encontrar o índice do menor elemento com base no nome
int findMin(String *idArray, int start, int end) {
    if (start == end)
        return start;

    int minIndexRest = findMin(idArray, start + 1, end);

    if (strcmp(idArray[start], idArray[minIndexRest]) < 0)
        return start;
    else
        return minIndexRest;
}
...
void selectionSort(String *idArray, int n, int index) {
    ...
    // Encontrar o índice do menor elemento com base no nome
    int minIndex = index;
    for (int i = index + 1; i < n; i++) {
        if (strcmp(idArray[i], idArray[minIndex]) < 0)
            minIndex = i;
    }
...
void bubbleSort(String *idArray, int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            Personagem p1 = getElementByID(idArray[j]);
            Personagem p2 = getElementByID(idArray[j + 1]);
            // Comparar pelo hairColour
            int hairCompare = strcmp(p1.hairColour, p2.hairColour);
            if (hairCompare > 0 || (hairCompare == 0 && strcmp(p1.name, p2.name) > 0)) {
...
int main(int argc, char const *argv[])
{
    ...
    clock_t startTime = clock();
     ...
     // Utilizando Bubble Sort baseado na cor do cabelo
    bubbleSort(idArray, x);
